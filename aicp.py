...
def parse_args(argv: List[str]) -> Dict[str, Any]:
    ...
    while i < len(argv):
        ...
        if arg in ("-r", "--remote"):
            ...
            flags["json"] = True
        elif arg == "--llm-config":       # ★ 加这个
            if i + 1 < len(argv):
                flags["llm_config"] = argv[i + 1]
                i += 1
        elif arg == "--llm-config-file":
            if i + 1 < len(argv):
                flags["llm_config_file"] = argv[i + 1]
                i += 1
...
def main() -> None:
    ...
    if args["exec"]:
        ...
        # 优先用 --llm-config
        if args.get("llm_config_file"):
            try:
                with open(args["llm_config_file"], "r", encoding="utf-8") as f:
                    llm_config = json.load(f)
                    llm_config = resolve_env_vars(llm_config)
                    config = {"models": llm_config}
            except Exception as exc:
                _print_error(f"Failed to load --llm-config-file: {exc}", args["json"])
                sys.exit(1)
            config = {"models": llm_config}    
        elif args.get("llm_config"):
            try:
                llm_config = json.loads(args["llm_config"])
            except json.JSONDecodeError as exc:
                _print_error(f"Invalid --llm-config JSON: {exc}", args["json"])
                sys.exit(1)
            config = {"models": llm_config}
        else:
            # 回退到读 aicp.yaml
            try:
                ...
                _print_error(f"Failed to load config: {exc}", args["json"])
                sys.exit(1)

        try:
            # ★ 跳过远端能力探测
            aicp = AICP_LLM(config, skip_remote_caps=True)
        except Exception as exc:
            _print_error(f"Failed to init AICP_LLM: {exc}", args["json"])
            sys.exit(1)

        try:
            result = asyncio.run(
                aicp.chatEnvelop(
                    [{"role": "user", "content": args["exec"]}],
                    stream=False,
                )
            )
            ...
            if args["json"]:
                ...
                if payload.get("ok"):
                    ...
                    sys.stderr.write(f"ERROR: {payload.get('error', '未知错误')}\n")
                    sys.exit(1)

            sys.exit(0 if payload.get("ok") else 1)

        except Exception as exc:
            logger.exception("--exec mode failed")
            if args["json"]:
...
def _print_error(msg: str, as_json: bool):
    """辅助函数：统一错误输出"""
    if as_json:
