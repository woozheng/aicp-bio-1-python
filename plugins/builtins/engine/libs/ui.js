...
function Button(text, onClick, style) {
    ...
    // ★ 如果传了 onClick，用 addEventListener 绑定
    if (onClick) {
        btn.addEventListener('click', onClick);
        btn.addEventListener('touchend', function(e) { e.preventDefault(); onClick(); });
    }

    // ★ 兼容 LLM 生成的 btn.onClick = function() 写法
    Object.defineProperty(btn, 'onClick', {
        set: function(fn) {
            // 移除旧监听（如果有）
            if (this._clickHandler) {
                this.removeEventListener('click', this._clickHandler);
                this.removeEventListener('touchend', this._touchHandler);
            }
            this._clickHandler = fn;
            if (fn) {
                this._touchHandler = function(e) { e.preventDefault(); fn(); };
                this.addEventListener('click', fn);
                this.addEventListener('touchend', this._touchHandler);
            }
        },
        get: function() {
